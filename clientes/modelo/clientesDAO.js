  ...
  function seleccionarTabla(callback) {
    ...
    sql =
     `select  BIN_TO_UUID(id) as id,cedula,direccion,email,nombre,telefono,BIN_TO_UUID(municipio_id) as municipio_id 
      from clientes
      order by id`;
  ...
  function actualizarRegistro(dato, callback) {
    ...
    sql =
     `update clientes set  cedula = ?,direccion = ?,email = ?,nombre = ?,telefono = ?,municipio_id = UUID_TO_BIN(?) 
      where id = UUID_TO_BIN(?)`;
