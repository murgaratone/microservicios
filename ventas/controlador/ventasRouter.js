  ...
  const ventasDAO = require('../modelo/ventasDAO');
  ...
  router.get('/', (req, res) => {
    ventasDAO.seleccionarTabla((err,datos) => {
  ...
  router.get('/:id', (req, res) => {
      const { id } = req.params;
      ventasDAO.seleccionarRegistro(id,(err,dato) => {
  ...
  router.delete('/:id', (req, res) => {
    ...
    const { id } = req.params;
    ventasDAO.eliminarRegistro(id,(err,resultado) => {
      if (err) {
        if (err.code == 'ER_WRONG_VALUE_FOR_TYPE' || err.code == 'NOT_FOUND') {
          // Error del usuario
          res.status(STATUS_NOT_FOUND);
  ...
  router.post('/', (req, res) => {
    ...
    const { id,codigo,cliente_id,usuario_id,subtotal,iva,total } = req.body;

    const dato = {  id: id, codigo: codigo, cliente_id: cliente_id, usuario_id: usuario_id, subtotal: subtotal, iva: iva, total: total  };

    console.log(dato);

    ventasDAO.insertarRegistro(dato,(err,resultado) => {
  ...
  router.put('/:id', (req, res) => {
    ...
    const { codigo,cliente_id,usuario_id,subtotal,iva,total } = req.body;
    const { id } = req.params;

    const dato = {  id: id, codigo: codigo, cliente_id: cliente_id, usuario_id: usuario_id, subtotal: subtotal, iva: iva, total: total  };

    console.log(dato);
    ventasDAO.actualizarRegistro(dato,(err,resultado) => {
